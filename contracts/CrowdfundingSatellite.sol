...
contract CrowdfundingSatellite is Ownable, ReentrancyGuard {
    ...
    function createCampaign(uint256 _tokenId, address _creator, uint256 _goal, uint256 _duration, address _tokenAddress) external onlyOwner {
        ...
        emit CampaignCreated(_tokenId, _creator, _goal, block.timestamp + _duration, _tokenAddress);
    }
    ...
    function endCampaign(uint256 _tokenId) external onlyOwner {
        ...
        require(campaign.isActive, "Campaign is not active");
        require(
            block.timestamp >= campaign.endTime || campaign.totalFunded >= campaign.goal,
